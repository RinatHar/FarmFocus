...
import type { NewTaskFormValues } from "../Modal/Task/AddTaskModal";
import type { DifficultyTask } from "../../types/farm";

type Props = {
  register: UseFormRegister<NewTaskFormValues>;
  difficulty?: DifficultyTask;
};

export const TextAreaDescription = ({ register, difficulty }: Props) => {
  return (
    <div className="relative w-full group">
      <textarea
        ...
        rows={3}
        className={clsx(
          "peer h-full w-full pl-3 pr-4 pt-4 pb-2 text-sm rounded-lg shadow-md focus:outline-none transition-all duration-300 resize-none",
          {
      ...
      <label
        htmlFor="textarea"
        className={clsx(
          "absolute font-mono left-3 top-1 text-xs transition-all duration-200 ease-in-out cursor-text select-none pointer-events-none",
